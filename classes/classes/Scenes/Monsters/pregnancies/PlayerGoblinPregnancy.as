package classes.Scenes.Monsters.pregnancies 
{
	...
	/**
	 * Contains pregnancy progression and birth scenes for a Player impregnated by an Goblin.
	 */
	public class PlayerGoblinPregnancy implements VaginalPregnancy
	{
		...
		/**
		 * Create a new goblin pregnancy for the player.
		 * @param	pregnancyProgression instance used for registering pregnancy scenes
		 * @param	output instance for gui output
		 */
		public function PlayerGoblinPregnancy(pregnancyProgression:PregnancyProgression, output:GuiOutput) 
		{
			this.output = output;
			
			pregnancyProgression.registerVaginalPregnancyScene(PregnancyStore.PREGNANCY_PLAYER, PregnancyStore.PREGNANCY_GOBLIN, this);
		}
		...
		public function vaginalBirth():void 
		{
			...
			output.text(kGAMECLASS.images.showImage("birth-goblin"));
			...
			if (player.cor < 50) {
				output.text("You rue the day you encountered that horny goblin.  ");
			}
			...
			output.text("  As you ponder the implications, <b>you realize your breasts have been lactating</b>.  You are surprised by the thick streams of milk now cascading down your body.");
