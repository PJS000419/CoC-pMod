package classes.Scenes.Monsters.pregnancies 
{
	...
	public class PlayerImpHordePregnancy implements VaginalPregnancy
	{
		...
		public function updateVaginalPregnancy():Boolean 
		{
			...
			if (player.pregnancyIncubation === 380) {
				output.text("\n<b>Your belly is now more distended than a nine-month pregnancy.   You are definitely pregnant, but with what?</b>\n");
				displayedUpdate = true;
			}
			
			if (player.pregnancyIncubation === 216) {
				output.text("\n<b>The unmistakable bulge in your tummy is at least twice the size of that of a normal pregnancy.  ");
				player.createStatusEffect(StatusEffects.ImpHordeBrood, 0, 0, 0, 0);
				kGAMECLASS.dynStats("tou", -10, "spe", -50, "lib", 1, "sen", .5, "cor", 0);
				if (player.cor < 40) {
			...
			if (player.pregnancyIncubation === 180) {
				output.text("\n<b>The sudden impact of multiple kicks from inside your womb startle you.</b>\n");
				displayedUpdate = true;
			}
			
			if (player.pregnancyIncubation === 120) {
				output.text("\n<b>Your grotesquely distended belly makes your pregnancy draw eyes and hushed whispers from those around you.</b>\n");
				displayedUpdate = true;
			}
		...
		public function vaginalBirth():void 
		{
			...
			kGAMECLASS.dynStats("tou", 10, "spe", 52, "lib", 1, "sen", .5, "cor", 7);
